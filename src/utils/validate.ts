...
export const formSchema = z
  .object({
    ...
    reminderTime: z.string().refine((val) => REMINDER_TIMES.includes(val), {
      message: "Reminder time not valid",
    }),
  })
  .refine(
    (data) => {
      return parseInt(data.expirationTime) > parseInt(data.reminderTime);
    },
    { message: "Reminder time must be less than expiration time", path: ["reminderTime"]}
  );
